<header>
  <div class="container">
    <div class="bloc-logo-nav">
      <a href="pages/home/home.php">
        <img src="http://localhost/~matt/MyBO/asset/img/logo_final.png" alt="logo Planete Manga">
      </a>
    </div>
<!--    <div class="btn-menu-block">   -->
      <input type="checkbox" id="btn-menu" checked>
      <label for="btn-menu" >&#9776;</label>
    <!-- </div> -->
    <nav class="menu majuscule">
      <ul class="text-center">
        <li><a href="pages/home/home.php">Accueil</a></li>
        <li class="submenu"><a href="pages/home.php">L'école</a>
          <ul>
            <li><a href="pages/actu/actu.php">Actualité</a></li>
            <li><a href="pages/galerie/galerie.php">Galerie</a></li>
             <li><a href="pages/member/member.php">Carte Membre</a></li>
          </ul>
        </li>
        <li><a href="pages/cours/cours.php">Cours</a></li>
        <li><a href="pages/team/team.php">Equipe</a></li>
        <li class="submenu"><a href="pages/home.php">Divers</a>
          <ul>
            <li><a href="pages/event/event.php">Evénements</a></li>
            <li><a href="pages/press/press.php">Presse</a></li>
          </ul>
        </li>
        <li><a href="pages/partner/partner.php">Nos partenaires</a></li>
        <li><a href="admin/logout.php">Déconnexion</a></li>
      </ul>
    </nav>
  </div>
</header>
